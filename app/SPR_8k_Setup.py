...
def spr_setup_sheet(args=None):
    ...
    try:
        ...
        df_setup_trim = df_setup_ori.loc[:, ['Cmpd ID', 'MW', 'Plate_Barcode', 'Barcode', 'Test [Cpd] uM',
                                             'fold_dil', 'num_pts']]
            ...
            if header == 'Cmpd ID':
                ...
                for cmpd in range(nRows):
                    ...
                    for i in range(int(df_setup_trim.iloc[cmpd]['num_pts']) + 2):
                        ...
                        # As the SPR field limit is only 15 characters trim the cmpd's
                        if len(value) == 22:
                            v = value[:3] + '-' + value[9:13] + '_' + str(unique_cmpd)
                            list.append(v)
                        else:
                            v = value + '_' + str(unique_cmpd)
                            list.append(v)
                    unique_cmpd += 1
            else:
                for cmpd in range(nRows):
        ...
        create_lists(header='Cmpd ID', list=cmpd_list)
        ...
        if plate_bar:
            final_df = pd.DataFrame({'cmpd': cmpd_list, 'MW': mw_list, 'CONC': conc_list,
        ...
        # Reorder the columns
        if plate_bar:
            final_df = final_df.loc[:, ['cmpd', 'MW', 'CONC', 'BAR', 'PLATE_BAR']]
        else:
            final_df = final_df.loc[:, ['cmpd', 'MW', 'CONC', 'BAR']]
...
def save_output_file(df_final):
    ...
    # Save File
    save_file_path = os.path.join('Results', now + '_spr_setup_affinity_APPVersion_' + str(__version__))
    save_file_path = save_file_path.replace('.', '_')
    save_file_path = save_file_path + '.xlsx'

    try:
        df_final.to_excel(save_file_path)
        print('Setup File Saved Successfully!')

    except Exception as e:
        print(f'Issue saving file: {e}')
